...
控制台提供 6 个页签：实时调度数字孪生、基线实验基准、系统架构、年度路线图、工程能力图谱、
**模型路由方案**。前五个为算力侧，「实时调度数字孪生」页可热切换调度算法、调节多目标奖励权重，
并现场记录对比结果生成帕累托表；「模型路由方案」页可拖 λ 滑块实时看省钱比例与质量代价，
并给出按（能力域 × 难度档）的可交付推荐表 —— 全部计算在浏览器内完成，与 Python 端逐位一致。
...
- [x] **Q2** 多目标 DRL 算法设计与 PPO 训练 · 调度控制台 v1.0 · 场景外扩至模型路由 · 方案文件导出
      （K8s 插件顺延至 Q3，见 §三）
